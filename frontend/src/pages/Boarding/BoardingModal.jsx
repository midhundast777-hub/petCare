...
export const BoardingModal = ({ isOpen, onClose, booking, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (booking?.id) {
        await boardingService.updateBooking(booking.id, formData);
        addToast('Boarding reservation updated successfully!', 'success');
      } else {
        await boardingService.createBooking(formData);
        addToast('Boarding stay reserved successfully!', 'success');
      }
      if (onSaved) onSaved();
      onClose();
    } catch (err) {
      const msg = err.response?.data?.room?.[0] || err.response?.data?.expected_check_out_date?.[0] || 'Booking error. Room might be occupied during these dates.';
      addToast(msg, 'error');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={booking?.id ? 'Edit Boarding Booking' : 'Reserve Pet Boarding Suite'}
